...
export default function AdminPanel() {
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const ALL_APPS = ['desktop', 'app', 'scanner', 'scanner_admin', 'salesman', 'extension'];
      ALL_APPS.forEach(appKey => {
        newAllowedApps[appKey] = (newUserRole === 'admin' || appKey === newUserRole || (newUserRole === 'manager' && appKey === 'app'));
      });
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-6">
          <div>
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 px-2 py-2 rounded-lg transition-colors ${activeView === 'dashboard' ? 'bg-indigo-600/10 text-indigo-400' : 'hover:bg-slate-800 hover:text-white'}`}
            >
          ...
          <div>
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 px-2 py-2 rounded-lg transition-colors ${activeView === 'users' ? 'bg-indigo-600/10 text-indigo-400' : 'hover:bg-slate-800 hover:text-white'}`}
            >
              <Users className="w-5 h-5" /> Users
            </button>
            <button 
              onClick={() => setActiveView('roles')}
              className={`w-full flex items-center gap-3 px-2 py-2 rounded-lg transition-colors ${activeView === 'roles' ? 'bg-indigo-600/10 text-indigo-400' : 'hover:bg-slate-800 hover:text-white'}`}
            >
              <ShieldAlert className="w-5 h-5" /> Roles
            </button>
            <button 
              onClick={() => setActiveView('permissions')}
              className={`w-full flex items-center gap-3 px-2 py-2 rounded-lg transition-colors ${activeView === 'permissions' ? 'bg-indigo-600/10 text-indigo-400' : 'hover:bg-slate-800 hover:text-white'}`}
            >
              <Key className="w-5 h-5" /> Permissions
            </button>
            <button 
              onClick={() => setActiveView('pending')}
              className={`w-full flex items-center gap-3 px-2 py-2 rounded-lg transition-colors ${activeView === 'pending' ? 'bg-indigo-600/10 text-indigo-400' : 'hover:bg-slate-800 hover:text-white'}`}
            >
          ...
          <div>
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 px-2 py-2 rounded-lg transition-colors ${activeView === 'apps' ? 'bg-indigo-600/10 text-indigo-400' : 'hover:bg-slate-800 hover:text-white'}`}
            >
              <AppWindow className="w-5 h-5" /> Applications
            </button>
            <button 
              onClick={() => setActiveView('extensions')}
              className={`w-full flex items-center gap-3 px-2 py-2 rounded-lg transition-colors ${activeView === 'extensions' ? 'bg-indigo-600/10 text-indigo-400' : 'hover:bg-slate-800 hover:text-white'}`}
            >
      ...
      <div className="flex-1 flex flex-col h-full overflow-x-auto relative">
        ...
        <div className="flex-1 overflow-auto p-8 relative">
          ...
          {activeView === 'pending' && (
            <div className="max-w-7xl mx-auto flex flex-col h-full pb-8">
              <div className="flex items-center justify-between mb-6">
                <p className="text-slate-500">Review and approve new user registration requests</p>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-x-auto flex-1 relative">
                <table className="w-full text-left border-collapse">
                  <thead>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {Object.entries(users).filter(([k, u]) => u.role === 'pending').map(([key, u]) => (
                      <tr key={key} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className="text-xs font-semibold px-2.5 py-1 rounded-full flex w-fit items-center gap-1.5 bg-amber-100 text-amber-700">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end gap-2">
                            <button 
                              onClick={async () => {
                                try {
                                  await update(ref(database, `users/${key}`), { role: 'scanner' });
                                  alert('User approved and set to scanner role! You can edit their roles in the Users tab.');
                    ...
                    {pendingUsers === 0 && (
                      <tr>
                        <td colSpan={3} className="px-6 py-12 text-center text-slate-500 font-medium">No pending user requests.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
        ...
        {selectedUserKey && selectedUser && (
          <div className="absolute top-0 right-0 h-full w-[450px] bg-white shadow-2xl border-l border-slate-200 transform transition-transform duration-300 ease-in-out flex flex-col z-20">
            <div className="p-6 border-b border-slate-200 flex items-start justify-between bg-slate-50/50 flex-shrink-0">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-indigo-100 flex items-center justify-center font-bold text-indigo-700 text-xl border-2 border-white shadow-sm">
                  {selectedUser.email?.substring(0, 2).toUpperCase()}
                </div>
                <div>
                  <h3 className="text-lg font-bold text-slate-800">{selectedUser.email?.split('@')[0]}</h3>
                  <p className="text-sm text-slate-500">{selectedUser.email}</p>
                </div>
              </div>
              <button onClick={() => setSelectedUserKey(null)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors bg-white shadow-sm border border-slate-200">
                <X className="w-5 h-5" />
              </button>
            </div>
            
            <div className="flex border-b border-slate-200 px-6 pt-4 gap-6 bg-slate-50/50 flex-shrink-0">
              <button 
                onClick={() => setSlideoverTab('overview')}
                className={`pb-3 text-sm font-medium border-b-2 transition-colors ${slideoverTab === 'overview' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
              >
                Overview
              </button>
              <button 
                onClick={() => setSlideoverTab('apps')}
                className={`pb-3 text-sm font-medium border-b-2 transition-colors ${slideoverTab === 'apps' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
              >
                Applications Access
              </button>
              <button 
                onClick={() => setSlideoverTab('roles')}
                className={`pb-3 text-sm font-medium border-b-2 transition-colors ${slideoverTab === 'roles' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
              >
                Roles & Permissions
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-slate-50/30">
              {slideoverTab === 'overview' && (
                <div className="space-y-6">
                  <div>
                    <h4 className="text-sm font-bold text-slate-800 mb-1">Global Role Selection</h4>
                    <p className="text-xs text-slate-500 mb-4">Changing the role will instantly re-assign the standard apps and permissions associated with it.</p>
                    
                    <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                      <label className="block text-sm font-bold text-slate-700 mb-2">User Global Role</label>
                      <select
                        value={
                          selectedUser.role === 'app' ? 'manager' :
                          selectedUser.role === 'SALESPERSON' ? 'salesman' :
                          selectedUser.role || 'scanner'
                        }
                        onChange={(e) => handleInstantRoleChange(selectedUserKey, e.target.value)}
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-600 outline-none text-slate-800 font-medium"
                      >
                        <option value="scanner">Storekeeper / Scanner</option>
                        <option value="salesman">Salesman</option>
                        <option value="manager">App / Manager</option>
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-slate-800 mb-2">User Details</h4>
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-x-auto text-sm">
                      <div className="grid grid-cols-3 border-b border-slate-100 p-3">
                        ...
                        <span className="text-slate-800 font-semibold col-span-2">{selectedUser.email}</span>
                      </div>
                      <div className="grid grid-cols-3 border-b border-slate-100 p-3 bg-slate-50/50">
                        <span className="text-slate-500 font-medium col-span-1">Password</span>
                        <span className="text-slate-800 font-mono tracking-wider col-span-2">{selectedUser.password || '├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó'}</span>
                      </div>
                      <div className="grid grid-cols-3 p-3">
                        <span className="text-slate-500 font-medium col-span-1">Status</span>
                        <span className="col-span-2">
                          <span className="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-bold text-xs uppercase tracking-wider">Active</span>
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              {slideoverTab === 'apps' && (
                <div className="space-y-6">
                  <div>
                    <h4 className="text-sm font-bold text-slate-800 mb-1">Applications Access</h4>
                    <p className="text-xs text-slate-500 mb-4">Manage which applications this user can access.</p>
                    
                    <div className="space-y-3">
                      {[
                        { key: 'scanner', title: 'Scanner Mobile App', desc: 'Mobile app for scanning operations', icon: Smartphone, color: 'text-indigo-600', bg: 'bg-indigo-50' },
                        { key: 'scanner_admin', title: 'Scanner Admin App', desc: 'Admin panel for scanner management', icon: ShieldAlert, color: 'text-teal-600', bg: 'bg-teal-50' },
                        { key: 'salesman', title: 'Salesman Mobile App', desc: 'Mobile app for sales representatives', icon: Smartphone, color: 'text-emerald-600', bg: 'bg-emerald-50' },
                        { key: 'app', title: 'Inventory Application', desc: 'Inventory management desktop/mobile app', icon: Monitor, color: 'text-blue-600', bg: 'bg-blue-50' },
                        { key: 'desktop', title: 'Desktop Python App', desc: 'Python automation suite access', icon: Monitor, color: 'text-rose-600', bg: 'bg-rose-50' },
                        { key: 'extension', title: 'Sticker Printer Extension', desc: 'Sticker printing extension', icon: Printer, color: 'text-pink-600', bg: 'bg-pink-50' },
                      ].map((app) => {
                        const hasAccess = !!selectedUser.allowedApps?.[app.key as keyof NonNullable<CustomUser['allowedApps']>];
                        return (
                          <div key={app.key} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${hasAccess ? 'border-indigo-200 bg-white shadow-sm' : 'border-slate-200 bg-slate-50/50 grayscale-[0.3]'}`}>
                            <div className="flex items-center gap-3">
                              <div className={`w-10 h-10 rounded-lg ${app.bg} ${app.color} flex items-center justify-center`}>
                                <app.icon className="w-5 h-5" />
                              </div>
                              <div>
                                <p className={`text-sm font-bold ${hasAccess ? 'text-slate-800' : 'text-slate-600'}`}>{app.title}</p>
                                <p className="text-xs text-slate-500">{app.desc}</p>
                              </div>
                            </div>
                            <button 
                              onClick={() => toggleAppAccess(selectedUserKey, selectedUser, app.key as any)}
                              className={`w-11 h-6 rounded-full transition-colors relative flex items-center ${hasAccess ? 'bg-indigo-600' : 'bg-slate-300'}`}
                            >
                              <div className={`w-4 h-4 bg-white rounded-full absolute transition-transform ${hasAccess ? 'translate-x-6' : 'translate-x-1'}`}></div>
                            </button>
                          </div>
                        );
                      })}
                      
                      {/* Special mapping for permissions as apps in the UI */}
                      {[
                        { perm: 'salesman_admin', title: 'Salesman Admin App', desc: 'Admin panel for sales management', icon: ShieldAlert, color: 'text-amber-600', bg: 'bg-amber-50' },
                        { perm: 'inventory_admin', title: 'Inventory Admin App', desc: 'Admin panel for inventory management', icon: ShieldAlert, color: 'text-emerald-600', bg: 'bg-emerald-50' },
                      ].map((app) => {
                        const hasAccess = !!selectedUser.permissions?.[app.perm as keyof NonNullable<CustomUser['permissions']>];
                        return (
                          <div key={app.perm} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${hasAccess ? 'border-indigo-200 bg-white shadow-sm' : 'border-slate-200 bg-slate-50/50 grayscale-[0.3]'}`}>
                            <div className="flex items-center gap-3">
                              <div className={`w-10 h-10 rounded-lg ${app.bg} ${app.color} flex items-center justify-center`}>
                                <app.icon className="w-5 h-5" />
                              </div>
                            ...
                            <button 
                              onClick={() => togglePermission(selectedUserKey, selectedUser, app.perm as any)}
                              className={`w-11 h-6 rounded-full transition-colors relative flex items-center ${hasAccess ? 'bg-indigo-600' : 'bg-slate-300'}`}
                            >
                              <div className={`w-4 h-4 bg-white rounded-full absolute transition-transform ${hasAccess ? 'translate-x-6' : 'translate-x-1'}`}></div>
                            </button>
              ...
              {slideoverTab === 'roles' && (
                <div className="space-y-4">
                  <h4 className="text-sm font-bold text-slate-800 mb-2">Granular Permissions</h4>
                  <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-2">
                    {[
                      { key: 'reports', label: 'View Reports' },
                      { key: 'invoices', label: 'Manage Invoices' },
                      { key: 'request_forms', label: 'Manage Request Forms' },
                      { key: 'reminders', label: 'Access Reminders' },
                      { key: 'notes', label: 'Access Notes' },
                      { key: 'profile', label: 'Edit Profile Settings' },
                      { key: 'scanner_tracking', label: 'Scanner Tracking' },
                      { key: 'app_hub', label: 'Access App Hub' }
                    ].map((perm) => (
                      <div key={perm.key} className="flex items-center justify-between py-3 px-4 border-b border-slate-100 last:border-0 hover:bg-slate-50 rounded-lg transition-colors">
                        <span className="text-sm font-medium text-slate-700">{perm.label}</span>
                        <button 
                          onClick={() => togglePermission(selectedUserKey, selectedUser, perm.key as any)}
                          className={`w-9 h-5 rounded-full transition-colors relative flex items-center ${selectedUser.permissions?.[perm.key as keyof NonNullable<CustomUser['permissions']>] ? 'bg-emerald-500' : 'bg-slate-300'}`}
                        >
                          <div className={`w-3.5 h-3.5 bg-white rounded-full absolute transition-transform ${selectedUser.permissions?.[perm.key as keyof NonNullable<CustomUser['permissions']>] ? 'translate-x-4' : 'translate-x-1 shadow-sm'}`}></div>
                        </button>
                      </div>
                    ))}
                  </div>
            ...
            <div className="p-6 border-t border-slate-200 bg-white flex justify-between items-center flex-shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
              <button
                onClick={() => handleDeleteUser(selectedUserKey)}
                className="px-4 py-2.5 text-red-600 hover:bg-red-50 rounded-lg text-sm font-bold transition-colors"
              >
                Delete User
              </button>
              <div className="flex gap-3">
                ...
                onClick={() => {
                  setStagedChanges(prev => {
                    const next = { ...prev };
                    delete next[selectedUserKey];
                    return next;
                  });
                  setSelectedUserKey(null);
                }}
                className="px-4 py-2.5 border border-slate-300 text-slate-700 rounded-lg text-sm font-bold hover:bg-slate-50 transition-colors"
              >
                Cancel
              </button>
              <button 
                onClick={() => {
                  if (stagedChanges[selectedUserKey]) {
                    handleSaveUser(selectedUserKey);
                  }
                  setSelectedUserKey(null);
                }}
                className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm shadow-indigo-200"
                disabled={!stagedChanges[selectedUserKey]}
              >
                Save Changes
              </button>
              </div>
            ...
            <form onSubmit={handleAddUser} className="p-6 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-600 outline-none text-slate-800 placeholder-slate-400 transition-shadow"
                  placeholder="├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó├óΓé¼┬ó"
                  required
                />
              </div>
              <div>
              ...
              <form onSubmit={async (e) => {
                ...
                <div>
                  ...
                  <select value={editingUser.role} onChange={(e) => setEditingUser({...editingUser, role: e.target.value})} className="w-full px-4 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-600 outline-none text-slate-800 bg-white transition-shadow">
                    ...
                    <option value="storekeeper">Inventory Storekeeper</option>
