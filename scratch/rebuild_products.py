  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-4 bg-white border border-slate-200 p-5 rounded-3xl shadow-sm self-start">
        <h3 className="font-bold text-slate-800 text-sm mb-1">Add to Inventory</h3>
        <p className="text-xs text-slate-400 mb-4">Register new SKUs for field sales.</p>
        <div className="space-y-4">
          <div>
            <label className="text-xs font-bold text-slate-500 mb-1 block">Product Name</label>
            <input 
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-500 mb-1 block">SKU Code</label>
              <input 
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="col-span-1">
              <label className="text-xs font-bold text-slate-500 mb-1 block">Stock</label>
              <input 
                type="number"
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                placeholder="0"
      ...
      <div className="lg:col-span-8 bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            <h3 className="font-bold text-slate-800">Master Inventory</h3>
                ...
                <tr key={p.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    {editingProduct?.id === p.id ? (
                      ...
                      <div className="font-mono font-bold text-slate-700">{p.stock}</div>
                    )}
                  </td>
                  <td className="px-5 py-4 text-right">
                    {editingProduct?.id === p.id ? (
                      <div className="flex justify-end space-x-2">
                        ...
                        <button onClick={() => setEditingProduct(null)} className="p-1.5 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200"><X className="w-4 h-4" /></button>
                      </div>
                    ) : (
                      <div className="flex justify-end space-x-2">
...
with open(os.path.join(dest_dir, "Products.tsx"), "w", encoding="utf-8") as f:
    f.write(products_code)

print("Products.tsx built")
