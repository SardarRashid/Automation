...
orders_code = """import React, { useState, useMemo } from 'react';
...
import { Order, Customer, Product } from '../../../types/SalesmanAdmin';
...
export default function Orders() {
  ...
  const handleUpdateOrderStatus = async (orderId: string, status: string) => {
    if (confirm(`Change order status to ${status}?`)) {
      await fbService.updateOrderStatus(orderId, status);
    }
  };

  const handleConfirmDelivery = async () => {
    if (deliverConfirmOrder) {
      await fbService.deductStock(deliverConfirmOrder, products);
      await fbService.updateOrderStatus(deliverConfirmOrder.id, 'Delivered');
  ...
  const handleProcessPayment = async () => {
    if (paymentModalOrder && paymentModalAmount) {
      await fbService.processPayment(paymentModalOrder, Number(paymentModalAmount), paymentModalMethod, customers);
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
      <div className="p-5 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between bg-slate-50/50 gap-4">
        <div>
          <h3 className="font-bold text-slate-800">Order Management HQ</h3>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input 
            type="text"
            placeholder="Search Order ID, Client, Salesman..."
            className="border border-slate-200 rounded-xl px-3 py-1.5 text-xs w-56 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
            value={adminOrderSearchText}
            onChange={(e) => setAdminOrderSearchText(e.target.value)}
          />
          <input 
            type="date"
            className="border border-slate-200 rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-slate-600"
            value={adminOrderDateFilter}
            onChange={(e) => setAdminOrderDateFilter(e.target.value)}
          />
          <select 
            className="border border-slate-200 rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-medium text-slate-700"
            ...
            {filteredOrders.map(o => (
              <tr key={o.id} className="hover:bg-slate-50/80 transition-colors group">
                <td className="px-5 py-4">
                  <div className="font-mono text-xs font-bold text-slate-800">{o.id}</div>
                  <div className="text-[11px] text-slate-400 mt-0.5">{o.date} {o.time}</div>
                </td>
                <td className="px-5 py-4">
                  <div className="font-medium text-slate-700">{o.salespersonName}</div>
                </td>
                <td className="px-5 py-4">
                  <div className="font-bold text-blue-900">{o.customerName}</div>
                </td>
                <td className="px-5 py-4">
                  <div className="font-bold text-slate-800">{Number(o.totalAmount).toFixed(2)} SAR</div>
                  <div className="text-[10px] text-slate-400">{o.items.length} items</div>
                </td>
                <td className="px-5 py-4">
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-100">
            <div className="p-6 bg-slate-50 border-b border-slate-100 text-center">
              ...
              <h3 className="font-black text-xl text-slate-800">Confirm Delivery</h3>
              <p className="text-sm text-slate-500 mt-2">
                Order <span className="font-mono text-slate-800 font-bold">{deliverConfirmOrder.id}</span>
              </p>
            </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-100">
            <div className="p-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              <div>
                <h3 className="font-black text-xl text-slate-800">Record HQ Payment</h3>
                <p className="text-xs text-slate-500 mt-1">
                  Order <span className="font-mono text-slate-800 font-bold">{paymentModalOrder.id}</span>
                </p>
              </div>
              <button onClick={() => setPaymentModalOrder(null)} className="p-2 bg-slate-200 text-slate-600 rounded-full hover:bg-slate-300 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 mb-1 block">Payment Amount Received (SAR)</label>
                <div className="relative">
                  ...
                  <input 
                    type="number" 
                    className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-11 pr-4 py-3 text-lg font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                    placeholder="0.00"
...
with open(r"D:\AntiGravity\Latest_Active_Apps\inventory-web-workspace\frontend\src\pages\salesman-admin\components\Orders.tsx", "w", encoding="utf-8") as f:
    f.write(orders_code)

print("Orders.tsx built")
