...
import type {  User  } from '../../../types/SalesmanAdmin';
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-5 rounded-3xl shadow-sm self-start">
        <h3 className="font-bold text-slate-800 dark:text-slate-100 text-sm mb-1">Enroll Route Salesman</h3>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 block">Email Address (Login ID)</label>
            <input 
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
          ...
          <div>
            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 block">Password (Onboarding)</label>
            <input 
              type="text"
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
          ...
          <div>
            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 block">Assigned Territory</label>
            <select 
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
          ...
          <div className="flex items-center space-x-2 pt-1">
            ...
            />
            <label htmlFor="priceOverride" className="text-sm text-slate-600 dark:text-slate-300 font-medium">Allow manual price override on POS</label>
          </div>
          <button 
      ...
      <div className="lg:col-span-8 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
          <div>
            <h3 className="font-bold text-slate-800 dark:text-slate-100">Active Field Force</h3>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {users.filter(u => u.role === 'SALESPERSON').map(u => (
                <tr key={u.id} className="hover:bg-slate-50 dark:bg-slate-900/80 transition-colors">
                  ...
                  <td className="px-5 py-4 text-right">
                    {editingUser?.id === u.id ? (
                      <div className="flex justify-end space-x-2">
                        ...
                        <button onClick={() => setEditingUser(null)} className="p-1.5 bg-slate-100 dark:bg-slate-700/50 text-slate-700 dark:text-slate-200 rounded-lg hover:bg-slate-200">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    ) : (
