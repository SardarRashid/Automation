...
import type {  Customer  } from '../../../types/SalesmanAdmin';
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-5 rounded-3xl shadow-sm self-start">
        <h3 className="font-bold text-slate-800 dark:text-slate-100 text-sm mb-1">Onboard Client</h3>
        <p className="text-xs text-slate-400 mb-4">Register new shops for field sales.</p>
        <div className="space-y-4">
          <div>
            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 block">Owner/Contact Name</label>
            <input 
          ...
          <div>
            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 block">Shop/Business Name</label>
            <input 
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 block">Phone</label>
              <input 
                className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
            ...
            <div>
              <label className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 block">Opening Balance</label>
              <input 
                type="number"
                className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                placeholder="0.00"
          ...
          <div>
            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1 block">Address</label>
            <input 
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
      ...
      <div className="lg:col-span-8 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
          <div>
            <h3 className="font-bold text-slate-800 dark:text-slate-100">Client Directory</h3>
            <p className="text-xs text-slate-400">Manage shops and view outstanding balances.</p>
          </div>
          <div className="flex space-x-2">
            <button onClick={handleExportCustomers} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 px-3 py-1 rounded-full text-xs font-bold flex items-center space-x-1 hover:bg-slate-50 dark:bg-slate-900">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {customers.map(c => (
                <tr key={c.id} className="hover:bg-slate-50 dark:bg-slate-900/80 transition-colors">
                  <td className="px-5 py-4">
                    {editingCustomer?.id === c.id ? (
                      ...
                      <div className="font-bold text-slate-800 dark:text-slate-100">{c.shopName}</div>
                    )}
                  </td>
                  <td className="px-5 py-4">
                    {editingCustomer?.id === c.id ? (
                  ...
                  <td className="px-5 py-4">
                    {editingCustomer?.id === c.id ? (
                      ...
                      <div className="text-slate-600 dark:text-slate-300 text-xs truncate max-w-[150px]" title={c.address}>{c.address}</div>
                    )}
                  </td>
                  <td className="px-5 py-4">
                    {editingCustomer?.id === c.id ? (
                  ...
                  <td className="px-5 py-4 text-right">
                    {editingCustomer?.id === c.id ? (
                      <div className="flex justify-end space-x-2">
                        ...
                        <button onClick={() => setEditingCustomer(null)} className="p-1.5 bg-slate-100 dark:bg-slate-700/50 text-slate-700 dark:text-slate-200 rounded-lg hover:bg-slate-200"><X className="w-4 h-4" /></button>
                      </div>
                    ) : (
                      <div className="flex justify-end space-x-2">
