  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl shadow-sm overflow-hidden">
      <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
        <div>
          <h3 className="font-bold text-slate-800 dark:text-slate-100">Field Collections (Payments)</h3>
            ...
            {payments.slice().reverse().map(p => (
              <tr key={p.id} className="hover:bg-slate-50 dark:bg-slate-900/80 transition-colors">
                <td className="px-5 py-4 whitespace-nowrap">
                  <div className="text-slate-800 dark:text-slate-100 font-medium">{new Date(p.date).toLocaleDateString()}</div>
                ...
                <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{p.collectedBy}</td>
                <td className="px-5 py-4 font-bold text-emerald-600">{Number(p.amountPaid).toFixed(2)} SAR</td>
                <td className="px-5 py-4 text-xs text-slate-500 dark:text-slate-400 truncate max-w-[150px]" title={p.description}>{p.description}</td>
