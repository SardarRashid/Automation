...
import type { InventoryRecord, CategoryTemplate, Storekeeper } from "../types";
import { 
  getRecordsByDate, 
  saveRecord, 
  deleteRecord, 
...
import { approveStockCount, rejectStockCount } from "../services/stockCountService";
import type { StockCount } from "../services/stockCountService";
...
export function DailySheetView({ selectedDate, onDateChange, categories, currentUser }: DailySheetViewProps) {
  const isFullAccess = ['supervisor', 'it_admin', 'admin', 'office'].includes(currentUser.role);
  ...
  // 'verify-counts' = Verify Pending Store Counts
  const [subTab, setSubTab] = useState<'arrivals' | 'audit' | 'master' | 'progress-feed' | 'verify-counts'>('verify-counts');

  const [pendingCounts, setPendingCounts] = useState<StockCount[]>([]);
  ...
  useEffect(() => {
    async function initStores() {
      try {
        ...
        console.error("Error loading store rooms in daily sheet:", err);
      }
    }
    initStores();
  }, []);
  ...
  const assignedStoresList = React.useMemo(() => {
    if (isFullAccess || !currentUser.assignedStoreNum || currentUser.assignedStoreNum === "All") {
      if (storeRooms.length > 0) {
        return storeRooms.map(r => r.name);
      }
      return ["Container-1", "Container-2", "Store 1", "Store 2", "Store 3", "Store 4", "Store 5", "Store 6", "Vegetable Area", "Flower Cooler"];
    }
    return currentUser.assignedStoreNum.split(",").map(s => s.trim()).filter(Boolean);
  }, [currentUser.assignedStoreNum, currentUser.role, storeRooms]);

  // Under Room-First assignment, storekeepers automatically see all categories 
  ...
  useEffect(() => {
    if (activeTemplate) {
      ...
      // Default container location name based on assigned stores
      setFastLocation(assignedStoresList[0] || `Container-${activeTemplate.name.substring(0, 3).toUpperCase()}1`);
    }
  }, [fastCat, categories, assignedStoresList]);
  ...
  useEffect(() => {
    ...
    const unsub = onValue(countsRef, (snap) => {
      if (snap.exists()) {
        const data = snap.val();
        const counts = Object.keys(data).map(k => data[k] as StockCount);
        // Only show pending counts for the selected date
        setPendingCounts(counts.filter(c => c.status === 'Pending Verification' && c.date === selectedDate));
      } else {
        setPendingCounts([]);
      }
    });
  ...
  const handleRejectCount = async (count: StockCount) => {
    if (!count.id) return;
    try {
      await rejectStockCount(count.id);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="hidden print:block mb-8 border-b-2 border-slate-900 pb-5">
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Muhammad Abdullah Sharbatly Fruit • Cold Storage Operations</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 mt-5 border-t border-slate-300 pt-4 text-xs text-slate-700">
          <div>
            <span className="text-slate-400 uppercase font-black block tracking-wider text-xs">Audit Date:</span>
            <span className="font-extrabold text-slate-900">{selectedDate}</span>
          </div>
          <div>
            <span className="text-slate-400 uppercase font-black block tracking-wider text-xs">Assigned Section:</span>
            <span className="font-extrabold text-slate-900">{currentUser.assignedSection || "All"}</span>
          </div>
          <div>
            <span className="text-slate-400 uppercase font-black block tracking-wider text-xs">Assigned Rooms/Stores:</span>
            <span className="font-extrabold text-slate-900">{currentUser.assignedStoreNum || "All"}</span>
          </div>
          <div>
            <span className="text-slate-400 uppercase font-black block tracking-wider text-xs">Report Inspector:</span>
        ...
        <div className="grid grid-cols-4 gap-3 mt-6 border-t border-slate-300 pt-4">
          <div className="border border-slate-300 rounded-lg p-2.5 text-center bg-slate-50/10">
            <span className="text-xs text-slate-500 uppercase font-black block tracking-wider">Total Varieties Checked</span>
            <span className="text-sm font-extrabold text-slate-900 font-mono">{filteredRecords.length}</span>
          </div>
          <div className="border border-slate-300 rounded-lg p-2.5 text-center bg-slate-50/10">
            <span className="text-xs text-slate-500 uppercase font-black block tracking-wider">Opening Stocks Sum</span>
            <span className="text-sm font-extrabold text-slate-900 font-mono">
              {filteredRecords.reduce((sum, r) => sum + (Number(r.openingStock) || 0), 0)} Pkgs
            </span>
          </div>
          <div className="border border-slate-300 rounded-lg p-2.5 text-center bg-slate-50/10">
            <span className="text-xs text-indigo-700 uppercase font-black block tracking-wider">Arrived (Incoming)</span>
          ...
          <div className="border border-slate-300 rounded-lg p-2.5 text-center bg-slate-50/10">
            <span className="text-xs text-green-700 uppercase font-black block tracking-wider">Ending Balance (In Store)</span>
            <span className="text-sm font-extrabold text-green-900 font-mono">
      ...
      <div className="bg-slate-200/70 p-1 rounded-xl flex flex-col md:flex-row max-w-4xl gap-1">
        ...
        <button
          onClick={() => setSubTab('verify-counts')}
          className={`flex-1 flex items-center justify-center gap-2 py-2 px-4 rounded-lg text-xs sm:text-sm font-semibold transition-all ${
            subTab === 'verify-counts'
              ? 'bg-purple-800 text-white shadow-md border-b-2 border-yellow-450'
              : 'text-slate-600 hover:text-green-800 hover:bg-white/40'
          }`}
        >
          <ClipboardCheck className="w-4.5 h-4.5" />
          <span>Verify Store Counts</span>
          {pendingCounts.length > 0 && (
            <span className="bg-red-500 text-white font-bold px-1.5 text-xs rounded-full">
        ...
        >
          ...
          {records.filter(r => r.incoming > 0).length > 0 && (
            <span className="bg-yellow-400 text-green-950 font-bold px-1.5 text-xs rounded-full">
      ...
      {/* Tab 0: VERIFY COUNTS MODE */}
      {subTab === 'verify-counts' && (
        <div className="space-y-4">
          <div className="bg-white/80 border border-slate-200/60 rounded-2xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">
              <ClipboardCheck className="w-5 h-5 text-purple-600" />
              Pending Store Counts for Verification
            </h3>
            {pendingCounts.length === 0 ? (
              <div className="text-center py-8 text-slate-500 text-sm">
                No pending counts to verify for {selectedDate}.
              </div>
            ) : (
                ...
                <table className="w-full text-left text-sm border-collapse">
                  <thead>
                    <tr className="bg-slate-100 text-slate-600 text-xs uppercase tracking-wider">
                      <th className="p-3 border-b border-slate-200 font-bold rounded-tl-lg">Time</th>
                      <th className="p-3 border-b border-slate-200 font-bold">Storekeeper</th>
                      <th className="p-3 border-b border-slate-200 font-bold">Store Room</th>
                      <th className="p-3 border-b border-slate-200 font-bold">Item & Variety</th>
                      <th className="p-3 border-b border-slate-200 font-bold">Physical Qty</th>
                      <th className="p-3 border-b border-slate-200 font-bold">Notes</th>
                      <th className="p-3 border-b border-slate-200 font-bold rounded-tr-lg text-center">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {pendingCounts.map((count) => (
                      <tr key={count.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="p-3 text-slate-500 font-mono text-xs border border-slate-200">
                          {new Date(count.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                        </td>
                        <td className="p-3 font-semibold text-slate-700 border border-slate-200">{count.storekeeperName}</td>
                        <td className="p-3 font-mono text-emerald-700 font-bold bg-emerald-50/50 border border-slate-200">{count.storeRoom}</td>
                        <td className="p-3 border border-slate-200">
                          <div className="font-bold text-slate-800">{count.category}</div>
                          <div className="text-xs text-slate-500">
                            {count.variety} (Size: {count.size})
                            {count.originCountry && ` - ${count.originCountry}`}
                            {count.grade && ` - ${count.grade}`}
                          </div>
                        </td>
                        <td className="p-3 font-mono font-bold text-blue-600 bg-blue-50/30 text-sm border border-slate-200">
                          {count.quantity}
                        </td>
                        <td className="p-3 text-slate-600 italic text-xs max-w-[150px] truncate border border-slate-200" title={count.notes}>
                          {count.notes || '-'}
                        </td>
                        <td className="p-3 text-center border border-slate-200">
                          <div className="flex justify-center gap-2">
                            <button
                              onClick={() => handleApproveCount(count)}
                              className="px-3 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded shadow-sm text-xs font-bold transition-all flex items-center gap-1 active:scale-95"
                            >
                              <CheckCircle2 className="w-3.5 h-3.5" /> Approve
                            </button>
                            <button
                              onClick={() => handleRejectCount(count)}
                              className="px-3 py-1.5 bg-red-100 hover:bg-red-200 text-red-700 border border-red-200 rounded shadow-sm text-xs font-bold transition-all active:scale-95"
                            >
                              Reject
                            </button>
                          </div>
          ...
          <div className="bg-gradient-to-r from-yellow-50/70 via-green-50/20 to-white border border-yellow-300/60 rounded-2xl p-5 shadow-inner">
            ...
            <form onSubmit={handleFastAddArrival} className="space-y-3.5">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end">
                ...
                {/* Cat */}
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">Fruit / Veg Type:</label>
                  <select
                    value={fastCat}
                  ...
                  >
                    {permittedCategories.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                  </select>
                </div>

                {/* Variety */}
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">Select Variety Option:</label>
                  <select
                    value={fastVariety}
                ...
                {/* Size */}
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">Product Size Count:</label>
                  <select
                    value={fastSize}
                ...
                {/* Location */}
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">Storage Loc. Container:</label>
                  <select
                    value={fastLocation}
                    onChange={(e) => setFastLocation(e.target.value)}
                    className="w-full bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 text-xs focus:ring-1 focus:ring-green-500 focus:outline-none font-sans"
                  >
                    {assignedStoresList.map(store => <option key={store} value={store}>{store}</option>)}
                  </select>
                </div>

                {/* Quantity arrived */}
                <div>
                  <label className="block text-xs font-semibold text-green-900 mb-1 font-sans">Shipment Weight/Qty:</label>
                  <input
                    type="number"
                    placeholder="e.g. 150"
                    min="1"
              ...
              <div className="bg-white/40 border border-slate-200/50 rounded-xl p-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Origin Country (Optional):</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Product Grade (Optional):</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1.5">Sub-variety / Brand Name (Optional):</label>
                  <input
                    type="text"
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs border border-slate-200">
                  <thead className="bg-slate-50 border-b border-indigo-50 text-slate-400 font-mono text-xs font-bold uppercase">
                    <tr>
                      <th className="py-2.5 px-4 border border-slate-200">Item Variety</th>
                      <th className="py-2.5 px-4 text-center border border-slate-200">Container No.</th>
                      <th className="py-2.5 px-4 text-center bg-sky-50/50 text-sky-800 font-bold border border-slate-200">SHIPMENT VALUE TODAY (What Came)</th>
                      <th className="py-2.5 px-4 text-center border border-slate-200">Current Total Stored</th>
                      <th className="py-2.5 px-4 border border-slate-200">Quality remarks or Notes</th>
                      <th className="py-2.5 px-4 text-center border border-slate-200">Action</th>
                    </tr>
                  </thead>
                    ...
                    {filteredRecords.map((rec, idx) => (
                      <tr key={rec.id || `rec-arr-${idx}`} className="hover:bg-sky-50/10 transition-colors">
                        <td className="py-3 px-4 border border-slate-200">
                          ...
                          <span className="ml-2 inline-block bg-slate-100 text-slate-500 text-sm px-1.5 rounded font-mono font-medium">#{rec.size}</span>
                        ...
                        <td className="py-3 px-4 text-center font-mono text-slate-500 border border-slate-200">{rec.location}</td>
                        
                        {/* INPUT: ONLY Shipment weight needs typing! */}
                        <td className="py-2 px-4 bg-sky-50/30 text-center border border-slate-200">
                          <div className="inline-flex items-center gap-1.5 justify-center">
                            <button
                              onClick={() => adjustCount(rec, "incoming", -5)}
                              className="bg-white border border-slate-200 hover:bg-slate-100 text-xs px-1 rounded text-slate-400 font-bold"
                            >
                              -5
                            </button>
                            <input
                            ...
                            <button
                              onClick={() => adjustCount(rec, "incoming", 5)}
                              className="bg-sky-50 border border-sky-200 hover:bg-sky-100 text-xs px-1 rounded text-sky-800 font-bold"
                            >
                              +5
                            </button>
                            <button
                              onClick={() => adjustCount(rec, "incoming", 25)}
                              className="bg-sky-600 text-white hover:bg-sky-700 text-sm px-1.5 py-1 rounded font-bold"
                              title="Add 25"
                            >
                              +25
                            </button>
                        ...
                        <td className="py-3 px-4 text-center font-bold font-mono text-slate-700 border border-slate-200">
        ...
        <div className="space-y-4">
          ...
          {filteredRecords.length > 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs border border-slate-200">
                  <thead className="bg-slate-50 border-b border-green-50 text-slate-400 font-mono text-xs font-bold uppercase">
                    <tr>
                      <th className="py-2.5 px-4 border border-slate-200">Item Variety</th>
                      <th className="py-2.5 px-4 text-center border border-slate-200">Container No.</th>
                      <th className="py-2.5 px-4 text-center bg-slate-100 text-slate-500 border border-slate-200">A. Yesterday's Balance (Opening)</th>
                      <th className="py-2.5 px-4 text-center bg-sky-50 text-sky-800 border border-slate-200">B. Arrived (Incoming)</th>
                      <th className="py-2.5 px-4 text-center bg-green-50 text-green-800 font-bold border border-slate-200">C. ENTER TODAY'S COUNT (Store Available)</th>
                      <th className="py-2.5 px-4 text-center bg-amber-50 text-amber-900 font-bold border border-slate-200">CALCULATED SALES (A + B - C)</th>
                      <th className="py-2.5 px-4 border border-slate-200">Quality conditions</th>
                      <th className="py-2.5 px-4 text-center border border-slate-200">Delete</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                    {filteredRecords.map((rec, idx) => {
                      ...
                      return (
                        <tr key={rec.id || `rec-vld-${idx}`} className="hover:bg-green-50/10 transition-colors">
                          <td className="py-3 px-4 border border-slate-200">
                            <span className="font-semibold text-slate-800">{rec.variety}</span>
                            <span className="ml-2 inline-block bg-slate-100 text-slate-450 text-sm px-1.5 rounded font-mono font-medium">#{rec.size}</span>
                            {(rec.originCountry || rec.grade || rec.subVariety) && (
                              <div className="flex flex-wrap gap-1 mt-1 text-sm">
                                {rec.originCountry && <span className="bg-slate-105 border border-slate-200 px-1 py-0.2 rounded text-slate-600">Origin: {rec.originCountry}</span>}
                                {rec.grade && <span className="bg-amber-50 border border-amber-200/50 text-amber-900 px-1 py-0.2 rounded font-bold">Grade: {rec.grade}</span>}
                          ...
                          <td className="py-3 px-4 text-center bg-slate-50/50 font-semibold font-mono text-slate-600 border border-slate-200">
                            {opening}
                          </td>

                          {/* Incoming today */}
                          <td className="py-3 px-4 text-center bg-sky-50/20 font-bold font-mono text-sky-700 border border-slate-200">
                            +{incoming}
                          </td>

                          {/* INPUT: Only enter current stock! System calculates sales! */}
                          <td className="py-1.5 px-4 bg-green-100/10 text-center border border-slate-200">
                            <div className="inline-flex items-center gap-1.5 justify-center">
                              <button
                                onClick={() => adjustCount(rec, "available", -1)}
                                className="bg-white border border-slate-200 hover:bg-slate-100 text-xs px-1.5 rounded font-bold text-slate-450"
                              >
                                -1
                              </button>
                              <input
                              ...
                              <button
                                onClick={() => adjustCount(rec, "available", 1)}
                                className="bg-green-50 border border-green-300 hover:bg-emerald-100 text-xs px-1.5 rounded font-bold text-green-800"
                              >
                                +1
                              </button>
                            </div>
                          ...
                          <td className="py-2 px-4 text-center bg-amber-50/30 border border-slate-200">
                            {calculatedSales > 0 ? (
                              <div className="inline-flex flex-col items-center">
                                ...
                                <span className="text-sm text-amber-600 font-semibold font-sans mt-0.5">Automated result</span>
                              </div>
                            ) : (
                          ...
                          <td className="py-1 px-4 border border-slate-200">
                            <input
                              type="text"
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm border-collapse border border-slate-200">
                  <thead className="bg-slate-50 border-b border-slate-150 text-slate-500 font-mono text-xs uppercase font-bold">
                    <tr>
                      <th className="py-3 px-4 min-w-[140px] border border-slate-200">Product / Variety</th>
                      <th className="py-3 px-4 text-center border border-slate-200">Size</th>
                      <th className="py-3 px-4 text-center border border-slate-200">Container No.</th>
                      <th className="py-3 px-4 text-center border border-slate-200">Arrived Date</th>
                      <th className="py-3 px-4 text-center bg-slate-50 text-slate-600 border border-slate-200">A. Opening Stock</th>
                      <th className="py-3 px-4 text-center bg-sky-50 text-sky-800 border border-slate-200">B. Arrived (+Incoming)</th>
                      <th className="py-3 px-4 text-center bg-amber-50 text-amber-800 border border-slate-200">C. Sold Today (-Outflow)</th>
                      <th className="py-3 px-4 text-center bg-green-50 text-green-800 font-bold border border-slate-200">D. Physical Total Ending (Available)</th>
                      <th className="py-3 px-4 min-w-[150px] border border-slate-200">Remarks / Quality notes</th>
                      <th className="py-3 px-4 text-center border border-slate-200">Manage</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 text-slate-700">
                    {filteredRecords.map((rec, idx) => (
                      <tr key={rec.id || `rec-all-${idx}`} className="hover:bg-slate-50/50">
                        <td className="py-3 px-4 font-semibold text-slate-800 border border-slate-200">
                          <div>{rec.variety}</div>
                          <div className="text-sm text-slate-450 font-mono">{rec.category}</div>
                          {(rec.originCountry || rec.grade || rec.subVariety) && (
                        ...
                        <td className="py-3 px-4 text-center font-bold text-slate-600 border border-slate-200">{rec.size}</td>
                        <td className="py-3 px-4 text-center border border-slate-200">
                          <input
                            type="text"
                            value={rec.location}
                            onChange={(e) => handleCellSave(rec, "location", e.target.value)}
                            className="bg-transparent border-b border-transparent hover:border-slate-300 text-center font-mono text-xs w-28 py-0.5 text-slate-700"
                          />
                        </td>
                        <td className="py-3 px-4 text-center text-slate-450 font-mono text-xs border border-slate-200">{rec.arrivalDate}</td>
                        
                        {/* A */}
                        <td className="py-2 px-2 bg-slate-50 text-center border border-slate-200">
                          <input
                            type="number"
                            min="0"
                            ...
                            className="bg-white border border-slate-350 py-1 text-center font-bold font-mono text-xs text-slate-800 w-16 rounded focus:outline-green-500"
                          />
                        </td>

                        {/* B */}
                        <td className="py-2 px-2 bg-sky-50/40 text-center border border-slate-200">
                          <input
                            type="number"
                            min="0"
                            value={rec.incoming}
                            ...
                            className="bg-white border border-slate-300 py-1 text-center font-bold font-mono text-xs text-sky-900 w-16 rounded focus:outline-green-500"
                          />
                        </td>

                        {/* C */}
                        <td className="py-2 px-2 bg-amber-50/40 text-center border border-slate-200">
                          <input
                            type="number"
                            min="0"
                            value={rec.sold}
                            ...
                            className="bg-white border border-slate-300 py-1 text-center font-bold font-mono text-xs text-amber-900 w-16 rounded focus:outline-green-500"
                          />
                        </td>

                        {/* D */}
                        <td className="py-2 px-2 bg-green-50 text-center border border-slate-200">
                          <input
                            type="number"
                            min="0"
                            value={rec.available}
                            ...
                            className="bg-green-50 border border-green-300 py-1 text-center font-extrabold font-mono text-xs text-green-950 w-16 rounded focus:outline-green-600 focus:border-green-400"
                          />
                        </td>
      ...
      <div className="hidden print:block mt-24 pt-8 border-t border-dashed border-slate-400">
        <div className="grid grid-cols-3 gap-8 text-center text-xs">
          <div>
            ...
            <p className="mt-2 text-slate-500 font-bold uppercase tracking-wider text-sm">Verified By (Storekeeper / Supervisor)</p>
          </div>
          <div>
            <div className="border-b border-slate-400 h-10 w-44 mx-auto" />
            <p className="mt-2 text-slate-500 font-bold uppercase tracking-wider text-sm">Checked By (Cold Storage Manager)</p>
          </div>
          <div>
            <div className="border-b border-slate-400 h-10 w-44 mx-auto" />
            <p className="mt-2 text-slate-500 font-bold uppercase tracking-wider text-sm">Authorized Audit Inspector</p>
          </div>
        </div>
        <div className="text-center mt-8 text-xs text-slate-400 font-medium font-mono uppercase tracking-widest">
