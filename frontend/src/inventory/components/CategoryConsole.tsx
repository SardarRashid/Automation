...
import type { CategoryTemplate, StoreRoom } from "../types";
...
export function CategoryConsole({ 
  ...
  const [newStoreCategories, setNewStoreCategories] = useState<string[]>([]);
  ...
  const handleAddSubGrade = async () => {
    if (!newSubGrade.trim() || !activeCategory) return;
    const cleanSubGrade = newSubGrade.trim();
    const subGradeList = activeCategory.subGrades || [];
    if (subGradeList.includes(cleanSubGrade)) {
      setFeedback("Sub-grade already exists in this category!");
      return;
    }
    const updatedCat = {
      ...activeCategory,
      subGrades: [...subGradeList, cleanSubGrade]
    };
    await onSaveCategory(updatedCat);
    setNewSubGrade("");
    showFeedback(`Added sub-grade "${cleanSubGrade}"`);
  };

  const handleRemoveSubGrade = async (subGrade: string) => {
    if (!activeCategory) return;
    const subGradeList = activeCategory.subGrades || [];
    const updatedCat = {
      ...activeCategory,
      subGrades: subGradeList.filter((g) => g !== subGrade)
    };
    await onSaveCategory(updatedCat);
    showFeedback(`Removed sub-grade "${subGrade}"`);
  };

  const handleAddSku = async () => {
    if (!newSku.trim() || !activeCategory) return;
    const cleanSku = newSku.trim();
    const skuList = activeCategory.skus || [];
    if (skuList.includes(cleanSku)) {
      setFeedback("SKU already exists in this category!");
      return;
    }
    const updatedCat = {
      ...activeCategory,
      skus: [...skuList, cleanSku]
    };
    await onSaveCategory(updatedCat);
    setNewSku("");
    showFeedback(`Added SKU "${cleanSku}"`);
  };
  ...
  const handleRemoveSku = async (sku: string) => {
    ...
    const updatedCat = {
      ...activeCategory,
      skus: skuList.filter((s) => s !== sku)
    };
    await onSaveCategory(updatedCat);
    showFeedback(`Removed SKU "${sku}"`);
  };
  ...
  const handleAddStoreRoom = async (e: React.FormEvent) => {
    ...
    const newRoom: StoreRoom = {
      ...
      allowedCategories: newStoreCategories.length > 0 ? newStoreCategories : undefined
    };

    await onSaveStoreRoom(newRoom);
    setNewStoreName("");
    setNewStoreDesc("");
    setNewStoreCategories([]);
  ...
  const downloadSampleCSV = () => {
    const csvContent = "Category,Variety,Size,Origin,Grade,Sub Variety,Sub Grade,SKU\n" +
      "Apple,Gala,100,Spain,Class 1,Red Gala,Premium,SKU-AP-GAL-100\n" +
      "Apple,Gala,113,Spain,Class 1,Red Gala,Standard,SKU-AP-GAL-113\n" +
      "Apple,Fuji,100,China,Extra Fancy,Striped,Premium,SKU-AP-FUJ-100\n" +
      "Garlic,Chinese,50mm,China,Class 1,White,Standard,SKU-GA-CHI-50\n" +
      "Garlic,Chinese,60mm,China,Class 1,White,Standard,SKU-GA-CHI-60\n" +
      "Grapes,Red Glob,Jumbo,Egypt,Premium,Seeded,Extra,SKU-GR-RED-JUM\n" +
      "Grapes,White Seedless,M,South Africa,Class 1,Thompson,Standard,SKU-GR-WHI-M";
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        let originIdx = headers.findIndex(h => h.includes("origin") || h.includes("country"));
        let gradeIdx = headers.findIndex(h => h === "grade" || h.includes("class"));
        let subVarIdx = headers.findIndex(h => h.includes("sub variety") || h.includes("sub-variety"));
        let subGradeIdx = headers.findIndex(h => h.includes("sub grade") || h.includes("sub-grade"));
        let skuIdx = headers.findIndex(h => h.includes("sku") || h.includes("barcode") || h.includes("code"));
        ...
        const categoriesMap = new Map<string, { 
          ...
          origins: Set<string>;
          grades: Set<string>;
          subVarieties: Set<string>;
          subGrades: Set<string>;
          skus: Set<string>;
        }>();
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          const originVal = originIdx !== -1 ? row[originIdx]?.trim() || "" : "";
          const gradeVal = gradeIdx !== -1 ? row[gradeIdx]?.trim() || "" : "";
          const subVarVal = subVarIdx !== -1 ? row[subVarIdx]?.trim() || "" : "";
          const subGradeVal = subGradeIdx !== -1 ? row[subGradeIdx]?.trim() || "" : "";
          const skuVal = skuIdx !== -1 ? row[skuIdx]?.trim() || "" : "";
          ...
          if (!categoriesMap.has(catId)) {
            categoriesMap.set(catId, {
              ...
              subVarieties: new Set<string>(),
              subGrades: new Set<string>(),
              skus: new Set<string>()
            });
          }
          ...
          if (originVal && originVal !== "-") catObj.origins.add(originVal);
          if (gradeVal && gradeVal !== "-") catObj.grades.add(gradeVal);
          if (subVarVal && subVarVal !== "-") catObj.subVarieties.add(subVarVal);
          if (subGradeVal && subGradeVal !== "-") catObj.subGrades.add(subGradeVal);
          if (skuVal && skuVal !== "-") catObj.skus.add(skuVal);
        }
        ...
        const parsedPreviews: ImportPreview[] = Array.from(categoriesMap.entries()).map(([id, data]) => {
          ...
          return {
            ...
            subVarieties: Array.from(data.subVarieties),
            subGrades: Array.from(data.subGrades),
        ...
        if (mergeMode) {
          ...
          if (existingCat) {
            finalCat = {
              id: parsed.id,
              name: parsed.name,
              varieties: Array.from(new Set([...existingCat.varieties, ...parsed.varieties])),
              sizes: Array.from(new Set([...existingCat.sizes, ...parsed.sizes])),
              origins: Array.from(new Set([...(existingCat.origins || []), ...(parsed.origins || [])])),
              grades: Array.from(new Set([...(existingCat.grades || []), ...(parsed.grades || [])])),
              subVarieties: Array.from(new Set([...(existingCat.subVarieties || []), ...(parsed.subVarieties || [])])),
              subGrades: Array.from(new Set([...(existingCat.subGrades || []), ...(parsed.subGrades || [])])),
              skus: Array.from(new Set([...(existingCat.skus || []), ...(parsed.skus || [])]))
            };
          } else {
            finalCat = {
              id: parsed.id,
              ...
              subVarieties: parsed.subVarieties,
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-4">
            <form onSubmit={handleAddStoreRoom} className="bg-slate-50 rounded-xl p-5 border border-slate-100 space-y-4">
              ...
              <div>
                <label className="block text-xs font-sans font-semibold text-slate-600 mb-2">
                ...
                <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto p-2 bg-white border border-slate-200 rounded-lg">
                  {categories.map(c => (
                    <label key={c.id} className="flex items-center gap-1.5 text-xs text-slate-700 cursor-pointer bg-slate-50 px-2 py-1 rounded border border-slate-100 hover:border-green-300">
                      <input 
                        type="checkbox"
                        checked={newStoreCategories.includes(c.id)}
                        onChange={(e) => {
                          if (e.target.checked) setNewStoreCategories([...newStoreCategories, c.id]);
                          else setNewStoreCategories(newStoreCategories.filter(id => id !== c.id));
                        }}
                        className="rounded border-slate-300 text-green-650 focus:ring-green-650"
                      />
                      {c.name}
                    </label>
                  ))}
                  {categories.length === 0 && <span className="text-slate-400 italic text-xs">No categories available</span>}
                </div>
              </div>
